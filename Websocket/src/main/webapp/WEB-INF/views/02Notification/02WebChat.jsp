...
<script type="text/javascript">
	...
	function wsMessage(event){
		...
		*/
	    ...
	    writeResponse(event.data); // 로그인창에 출력
	    
	    if(content == ""){
	        // 날라온 내용이 없음으로 아무것도 하지 않는다.
	    }else{
	        // 대화내용에 /가 포함되어 있다면 명령어로 판단한다. 
	        if(content.match("/")){
	        	/*
	        	현재 접속한 클라이언트의 아이디를 이용해서 본인한테 온 귓속말인지 판단한다.
	        	아래 chat_id는 형재 페이지 로드시 접속한 아이디를 통해 전역변수에 저장되어 있는 값이다.
	        	...
	        	*/
	        	...
	        	// 대화내용에 /가 포함되어 있지 않다면 일반적인 메세지로 판단한다.
	            msg = makeBalloon(sender, content);
	        	// 일반적인 대화내용은 모든 클라이언트에게 디스플레이 됨으로 대화창에 즉시 출력한다.
	            messageWindow.innerHTML += msg;
	            messageWindow.scrollTop = messageWindow.scrollHeight;
	        }
	    }
	...
	function enterkey(){
		// 키보드를 눌럿다가 땟을때 키코드값이 13일때만 전송함수를 호출한다.

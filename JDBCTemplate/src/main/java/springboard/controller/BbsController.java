...
public class BbsController {
	...
	/*
	 * @Autowired
	 * 		: 스프링 설정파일에서 이미 생성된 빈을 자동으로 주입받고 싶을때 사용한다.
	 * 		  타입을 기반으로 자동주입되며, 만약 해당 타입의 빈이 존재하지 않으면 에러가 발생되어 서버를 시작할 수 없다.
	 * 		  - 생성자, 멤버변수, 메서드(setter)에 적용할 수 있다.
	 * 		  - 타입을 이용해 자동으로 프로퍼티 값을 설정한다.
	 * 		  - 해당 어노테이션은 멤버변수에만 적용할 수 있따.
	 * 		    메서드 내의 지역변수에는 사용할 수 없다.
	 * 		  - 타입을 통해 자동으로 설정되믕로 같은 타입이 2개 이상 존재하면 예외가 발생한다.
	 */
	private JdbcTemplate template;
	/*
	 servlet-context.xml에서 생성한 빈을 여기서 자동으로 주입받는다.
	 해당 빈은 스프링 컨테이너가 시작될때 생성되며, 타입을 기반으로 자동 주입 받게된다.
	 */
	@Autowired
	...
	public String writeAction(Model model, HttpServletRequest req, SpringBbsDTO springBbsDTO) {
		...
		/*
		 글쓰기 페이지에서 전송된 모든 폼값은 SpringBbsDTO 객체를 통해 한꺼번에 받을 수 있다.
		 Spring 에서는 커맨드객체를 통해 이와같은 처리를 할 수 있다.
		 */
	...
	public String password(Model model, HttpServletRequest req	) {
		// 일련번호(idx)는 컨트롤러에서 파라미터를 받은 후 Model에 저장하여 View로 전달한다.
	...
	public String editAction(HttpServletRequest req, Model model, SpringBbsDTO springBbsDTO) {
		...
		/*
		 수정처리가 완료되면 상세페이지로 이동하게 되는데
		 이떄 idx와 같은 파라미터가 필요하다.

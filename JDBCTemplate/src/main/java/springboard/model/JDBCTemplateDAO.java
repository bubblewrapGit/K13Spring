...
/*
 ...
 Object queryForObject(String sql, RowMapper rm)
 	: 하나의 레코드나 결과값을 반환하는 select계열의 쿼리문을 실행할 때 사용한다.
...
public class JDBCTemplateDAO {
	...
	public JDBCTemplateDAO() {
		/*
		 ...
		 JdbcTemplateConst.template에 값을 할당하였음으로,
	...
	// JDBCTemplate에서는 자원 해제를 하지 않는다.
	public void close() {
		/*
		 JDBCTemplate에서는 자원해제를 하지 않는다.
		 Spring 설정파일에서 빈을 생성함으로 자원을 해제하면
		 다시 new를 통해 생성해야 함으로 자원해제를 사용하지 않는다.
		 */
	}
	...
	public ArrayList<SpringBbsDTO> list(Map<String, Object> map){
		...
		/*
		 RowMapper가 select를 통해 얻어온 ResultSet 을 DTO객체에 저장하고,
		 List컬렉션에 적재하여 반환한다.
		 그러므로 DAO에서 개발자가 반복적으로 하던 작업을 자동으로 처리해준다.
		 */
		return 
			(ArrayList<SpringBbsDTO>) template.query(sql, 
	...
	public int write(final SpringBbsDTO springBbsDTO) {
		...
		반드시 final을 선언하여 값이 변경이 불가능 하게 처리해야한다.
		final로 선언하지 않은면 에러가 발생한다.
		이것은 Java의 규칙이다.
		 */
		int result = template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				/*
				 하나의 쿼리문 내에서 nextval을 여러번 사용하더라도 항상 같은 시퀀스를 반환한다.
				 */
	...
	public void updateHit(final String idx) {
		...
		String sql = " UPDATE springboard SET "
				...
				+ " WHERE idx=? ";
		/*
		 행의 변화를 주는 쿼리문 실행임으로 update메서드를 사용한다.
	...
	public SpringBbsDTO view(String idx) {
		...
		try {
			/*
			 queryForObject() 메서드는 쿼리문을 실행한 후 반드시 하나의 결과를 반환해야한다.
			 그렇치 않으면 에러가 발생하게됨으로 예외처리를 하는것이 좋다.
			 */
			dto = template.queryForObject(sql, new BeanPropertyRowMapper<SpringBbsDTO>(SpringBbsDTO.class));
			/*
			 BeanPropertyRowMapper클래스는 쿼리의 실행결과를 DTO에 저장해주는 역할을 한다.

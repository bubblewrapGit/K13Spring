	...
	public static String pagingImg(
		...
		*/
		...
		if(intTemp != 1) {
			...
			//두번째 페이지 블럭부터 출력됨.
			pagingStr += ""
				+ "<a class='page-link' style='display:inline-block' href='"+page+"nowPage=1'>"
				+ "<i class='bi bi-skip-backward-fill'></i></a>";
			pagingStr += "&nbsp;";
			pagingStr += ""
				+ "<a class='page-link' style='display:inline-block' href='"+page+"nowPage="+
								(intTemp-blockPage)+"'>"
		...
		*/
		while(blockCount<=blockPage && intTemp<=totalPage)
		{
			if(intTemp==nowPage) {
				...
				pagingStr += "&nbsp;<a class='page-link' style='display:inline-block' "
					+ "href='"+page
					+"nowPage="+intTemp+"'>"+
					intTemp+"</a>&nbsp;";
			}
		...
		if(intTemp <= totalPage) {
			pagingStr += "<a class='page-link' style='display:inline-block' href='"+page+"nowPage="+
											intTemp+"'>"

...
public class ServiceController {
	/*
	 서비스객체를 주입받는다.
	 @Autowired는 멤버변수, 생성자, setter에서 사용할 수 잇다.
	 또한, 미리 생성되어 있어야 한다.
	 그렇치 않으면, 서버가 정상적으로 실행되지 않는다.
	 */
	MyService myService;
	@Autowired
	...
	/*
	 ...
	 이떄 컨트롤러가 받은 모든 요청(request객체)을 전달한다.
	 */
	...
	public String myLotto(LottoVO lottoVO, Model model) {
		/*
		 파라미터로 전달되는 값은 커맨드객체(VO객체)를 통해
		 한꺼번에 받을 수 있음으로 getParameter()가 필요없다.
		 */

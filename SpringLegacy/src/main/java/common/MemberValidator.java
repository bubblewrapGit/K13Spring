...
public class MemberValidator implements Validator{
	...
	/*
	 * 매개변수로 전달된 객체가 유효성 검증을 진행할 수 있는 커맨드객체인지 판단한다.
	 * 만약 해당 메서드를 통과하지 못하면 유효성 검증을 위한 validate()메서드는 아예 호출되지 않는다.
	 * 해당 메서드는 자동으로 호출된다.
	 */
	@Override
	...
	/*
	 * supports()가 true를 반환하는 경우에만 호출되는 메서드로
	 * 실제 폼값에 대한 검증을 진행한다.
	 * 매개변수1 : 폼값을 저장한 커맨드객체
	...
	public void validate(Object obj, Errors errors) {
		System.out.println("vlaidate() 메소드 호출됨");
		
		// Object형인 커맨드객체를 사용하기 위해 형변환 필요
		 ...
		 * validate()메서드 내부로 진입자체가 불가능함으로,
		 * 해당 함수에서 pw라는 문자열은 getter()메서드를 통해
	...
	// 사용자 정의 메서드 : 값이 null이거나 길이가 0일때 false반환
	public boolean myEmptyOrWhitespace(String value) {
		/*
		 * 단순한 빈값에 대한 검증 정도는 내장메서드를 사용하면 되겠지만
		 * 이보다 복잡한 실제 비즈니스 로직에서 사용하는 부분은 직접 정의해야 하는 경우가 많음으로 
		 * 사용자 정의 메서드가 필요하다.
		 */

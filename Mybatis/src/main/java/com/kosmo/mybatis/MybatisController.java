...
public class MybatisController {
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		방명록URL?nowPage=		-> "" => 페이지번호는 빈값
		방명록URL?nowPage=10	-> 10 => 10으로 설정
		방명록URL				->    => null로 판단
		 */
	...
	public String write(Model model, HttpSession session, HttpServletRequest req) {
		/*
		 ...
		 매개변수로 선언해준다. httpSession
		 */
		// session영역에 해당 속성이 없다면 로그아웃 상태임으로 로그인 페이지로 이동한다.
		if(session.getAttribute("siteUserInfo")==null) {
			/*
			 현재상태는 글쓰기를 위해 버튼을 클릭했음으로, 만약 로그인 완료된다면
			 글쓰기 페이지로 이동하는 것이 좋다.
			 따라서 backUrl이라는 파라미터에 쓰기페이지의 Viww경로를 붙여서 리다이렉트 시킨다.
			 */
			model.addAttribute("backUrl", "07Mybatis/write");
			return "redirect:login.do";
		}
		// 로그인이 완료된 상태라면 쓰기페이지로 이동한다.
	...
	public ModelAndView loginAction(HttpServletRequest req, HttpSession session	) {
		...
		if(vo==null) {
			...
			// 로그인에 성공한 경우 세션영역에 MemverVO객체를 저장한다.
	...
	public String writeAction(Model model, HttpServletRequest req, HttpSession session) {
		/*
		 글쓰기 페이지에 오랫동안 머물면 세션이 끊어지는 경우가 있음으로
		 글쓰기 처리에서도 반드시ㅣ 세션을 확인한 후 처리해야한다.
		 */
		if(session.getAttribute("siteUserInfo") == null	) {
	...
	public String modify(Model model, HttpServletRequest req, HttpSession session) {
		...
		/*
		 파라미터를 전달하는 4번째 방법으로 DTO(VO)객체에 파라미터를 저장한 후
		 Mapper로 전달하는 방식이다.
		 */
		ParameterDTO parameterDTO = new ParameterDTO();
		// 일련번호 저장
	...
	public String modifyAction(HttpSession session, MyBoardDTO myBoardDTO) {
		...
		// 방명력 게시판은 상세보기 페이지가 별도로 없음으로 리스트로 이동하면 된다.
		return "redirect:list.do";
	}
